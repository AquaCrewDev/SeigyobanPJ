   ...
   // width:1200,
    ...
    drawGraphReagion:function(date){
        ...
        */
        ...
        var y=this.yScale([0,40],[this.height,0]);
        var y2=this.yScale([0,40],[this.height,0]);
    ...
    /**
     * グラフの描画(updateLineGraphとの違いはidの付与とボタンを追加する点)
     * @param dataList               グラフ単体のデータ
     * @param date                   指定した日付
     * @param i                      グラフのid(0~31)
     * @return         null
    ...
    drawLineGraph: function(dataList,date,i){
        ...
        var y=this.yScale([0,40],[this.height+this.margin.top,0]);
        ...
        svg.append("path")
            ...
            .attr({
                d:line,
                "id":"cpath"+i,
                //buttonのgetElementByIdだと一つの要素しか選択できないためnameを付与
                //idの重複が起きているため回避が必要
                "name":"name_cpath"+i,
	...
	.on('click',function() {
      console.log(i);
      //nameを取得し、それぞれのグラフの表示を切り替え
		  var elm = document.getElementsByName("name_cpath"+i);
      for(j=0;j<elm.length;j++){
  	    //console.log(elm[j].style.visibility);
  			if(elm[j].style.visibility=='hidden'){
  			 	elm[j].style.visibility='visible';
     			//console.log(elm[j].style.visibility);
  			}else{
  				elm[j].style.visibility='hidden';
  			}
      }
	});
