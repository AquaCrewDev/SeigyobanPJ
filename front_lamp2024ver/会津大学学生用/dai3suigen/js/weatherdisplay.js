...
let icon_url = "https://www.jma.go.jp/bosai/forecast/img/";

//weatherCode対応関係
let weatherCode_contents = {
  "100": [
    "100.svg",
    "500.svg",
    "100",
...
fetch(url)
  ...
  .then(function(weather) {
    ...
    // document.getElementById("publishingOffice").lastElementChild.textContent = weather[1].publishingOffice;
    // document.getElementById("text").lastElementChild.innerHTML = weather[1].text.replace(/\n\n/g, '<br>');
    // 画面に書き出す
    // thread追加
    // $("#weather_contents").append("<thead><tr>");
    // $("#weather_contents").append("<th scope=\"col\" id=\"publishingOffice\"><td></td></th><th scope=\"col\" id=\"reportDatetime\"></th></tr>");
    let toDay = new Date(weather[0].reportDatetime);
    let tableParts =
    '<table class="table table-bordered text-center text-nowrap table-sm">'+
      '<thread>'+
    		'<tr>'+
    			'<td colspan="13" class="bg-primary">福島県会津の天気予報（明後日までの詳細）</td>'+
    		'</tr>'+
    		'<tr>'+
    			'<td colspan="13" class="info">' +toDay.getFullYear() +'年' +toDay.getMonth() +'月' +toDay.getDay() +'日' +toDay.getHours() +'時　' +weather[0].publishingOffice +' 発表 </td>'+
    		'</tr>'+
      '</thread>'+
      '<tbody>'+
    		'<tr class="info">'+
    			'<td>日付</td>'+
    			'<td colspan="4" class="font-weight-bold">今日</td>'+
    			'<td colspan="4" class="font-weight-bold">明日</td>'+
    			'<td colspan="4" class="font-weight-bold">明後日</td>'+
    		'</tr>';
    html += tableParts;
    ...
    let table_weather =
  		'<tr>'+
  			'<td rowspan="2">天気</td>';
    for (var i = 0; i <= 2; i++) {
      const weatherCode = weather[0].timeSeries[0].areas[2].weatherCodes[i];
      //console.log(weatherCode_contents.+$(weatherCode));
      //天気予報のSVG取得(週間分)
      if (weatherCode in weatherCode_contents) {
        //console.log(weatherCode_contents[weatherCode][0]);
        table_weather +=
        '<td colspan="4">'+
          '<img src=' + icon_url + weatherCode_contents[weatherCode][0] + '>'+
        '</td>';
      } else {
        console.log("not found weatherCode");
      }
    }
  	table_weather +=
      '</tr>'+
  		'<tr>';
    for (var i = 0; i <= 2; i++) {
  			table_weather += '<td colspan="4">'+nullcheck(weather[0].timeSeries[0].areas[2].weathers[i])+'</td>';
    }
  	table_weather +=
      '</tr>';
    html += table_weather;

    let table_rainypercent =
  		'<tr>'+
  			'<td rowspan="2">降水確率(%)</td>'+
        '<td>00-06</td>'+
        '<td>06-12</td>'+
        '<td>12-18</td>'+
        '<td>18-24</td>'+
    ...
    var j=0;
    for (var i = 0 ; i < 12; i++) {
      if((i*6)%24 == new Date(weather[0].timeSeries[1].timeDefines[j]).getHours()){
        table_rainypercent += '<td>'+weather[0].timeSeries[1].areas[2].pops[j]+'</td>';
        j++;
      }else{
        table_rainypercent += '<td>-</td>';
      }
    }
  	table_rainypercent +=	'</tr>';
    html += table_rainypercent;


    let table_temperature =
    		'<tr>'+
    			'<td rowspan="2">気温(℃)</td>'+
    ...
    var i=0;
    for (var i = 0 ; i < 6; i++) {
      const date = new Date(weather[0].timeSeries[2].timeDefines[i])
      if(i == 0){
        if(i == 0 && 9 == date.getHours()){
          table_temperature += '<td colspan="2">-</td>';
          table_temperature += '<td colspan="2">'+weather[0].timeSeries[2].areas[2].temps[i+1]+'</td>';
        }else{
          table_temperature += '<td colspan="2">'+weather[0].timeSeries[2].areas[2].temps[i]+'</td>';
          table_temperature += '<td colspan="2">'+weather[0].timeSeries[2].areas[2].temps[i+1]+'</td>';
        }
        i++;
      }else{
        table_temperature += '<td colspan="2">'+nullcheck(weather[0].timeSeries[2].areas[2].temps[i])+'</td>';
      }
    }
    table_temperature +=
        '</tr>'+
    ...
    html += table_temperature;
    document.getElementById('weather_contents').innerHTML = html;
  });
