// トピック名
const subtopic_image = ''; // ランプの色変更に関するAWS IoT Coreのトピック
...
function handleImageTopicMessage(payload) {
  ...
  if (payload && payload["RTC.TimedShortSeq"] && payload["RTC.TimedShortSeq"].data) {
    ...
    console.log("Received TimedShortSeq data:", data_list);
...
function handleRoboTopicMessage(payload) {
  if (payload && payload["RTC.TimedString"] && payload["RTC.TimedString"].data) {
    //const data_list = payload["RTC.TimedString"].data;

      // dataフィールドをJSONとしてパース
      const data_list = JSON.parse(payload["RTC.TimedString"].data);

      // data_listが配列であることを確認
      if (Array.isArray(data_list) && data_list.length > 0) {
        const firstEntry = data_list[0]; // 配列の最初の要素を取得
        if (firstEntry && firstEntry.position) { // positionプロパティの存在を確認
          const position = parseFloat(firstEntry.position); // 数値に変換
          
          console.log("Received position:", position);

          // positionを0~0.5の範囲に収める
          let position_x = isNaN(position) ? 0 : position;
